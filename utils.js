...
export const handleChangeLinkState = (linksElements, sections) => {
  ...
  const activeSectionIndex = Array.from(sections).findIndex(
    (section) =>
      ...
      currentScrollPosition < section.offsetTop + section.offsetHeight
  );
...
const getScrollContainer = (section) =>
  document.querySelector(`#${section} .${section}-container .cards-wrapper`);
const getCardWidth = (section) =>
  document.querySelector(`.${section}-container .card-container`).offsetWidth;
...
export const createScrollHandler = (section) => (event, direction) => {
  ...
  const scrollAmount =
    cardWidth * cardsToScroll * (direction === "next" ? 1 : -1);
  container.scrollLeft += scrollAmount;
};

export const handleFilterProject = (event, projectContainer, filter, filtersItems) => {
  const { projects } = DATA_CV;

  filtersItems.forEach(filter => filter.classList.remove('active-filter'));
  filter.classList.add('active-filter')
  document.querySelector('.cards-project-wrapper').remove();
  let filteredProjects = [];

  const selectedFilter = event.target.innerText;

  filteredProjects = projects.filter((project) =>
    project.technology.includes(selectedFilter.toLowerCase())
  );
  ...
  WrapperProjectCards(filteredProjects, projectContainer);
};
